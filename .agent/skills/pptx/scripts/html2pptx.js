...
function validateDimensions(bodyDimensions, pres) {
  ...
  if (pres.presLayout) {
    ...
    if (
      ...
      errors.push(
        `HTML dimensions (${widthInches.toFixed(1)}" × ${heightInches.toFixed(
          1
        )}") ` +
          `don't match presentation layout (${layoutWidth.toFixed(
            1
          )}" × ${layoutHeight.toFixed(1)}")`
      );
    }
  }
  return errors;
      ...
      if (fontSize > 12 && distanceFromBottom < minBottomMargin) {
        ...
        errors.push(
          `Text box "${textPrefix}" ends too close to bottom edge ` +
            `(${distanceFromBottom.toFixed(
              2
            )}" from bottom, minimum ${minBottomMargin}" required)`
        );
      }
    }
  }
  ...
  return await page.evaluate(() => {
    ...
    document.querySelectorAll("*").forEach((el) => {
      ...
      if (isContainer) {
        ...
        for (const node of el.childNodes) {
          if (node.nodeType === Node.TEXT_NODE) {
            ...
            if (text) {
              errors.push(
                `DIV element contains unwrapped text "${text.substring(0, 50)}${
                  text.length > 50 ? "..." : ""
                }". ` +
      ...
      if (el.tagName !== "LI" && /^[•\-\*▪▸○●◆◇■□]\s/.test(text.trimStart())) {
        errors.push(
          `Text element <${el.tagName.toLowerCase()}> starts with bullet symbol "${text.substring(
            0,
            20
